...
def getFrequencyDict(SCRABBLE_LETTER_VALUES):
    """
...
def playGame(wordList):
    ...
    while True:
        ask = input("Input 'n' for a new hand, 'r' to play the last hand, 'e' to exit the game: ").lower()
        if ask == "n":
            numberGames+=1
            hand = dealHand(n)
            hand_now = hand.copy()
            ...
            if numberGames == 0:
                print("You have not played any hand yet")
                ask = input("Input 'n' for a new hand, 'r' to play the last hand, 'e' to exit the game: ").lower()
            else:
                hand = hand_now
                playHand(hand, wordList, n)
...
if __name__ == '__main__':
    wordList = loadWords()
    #I assigned it because when i ran the code without it , i threw an error saying "hand is not defined"
playGame(wordList)   
